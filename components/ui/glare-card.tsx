...
export const GlareCard = ({
  ...
  return (
    ...
    >
      <div className="h-full w-full grid [transform-style:preserve-3d] [transform:rotateY(var(--r-y))_rotateX(var(--r-x))] will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)]">
        <div className="w-full h-full grid [grid-area:1/1] overflow-hidden rounded-[var(--radius)] bg-slate-950 border border-slate-800">
          <div className="h-full w-full">{children}</div>
        </div>
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] opacity-[var(--opacity)] transition-opacity transition-background duration-[var(--duration)] ease-[var(--easing)] hover:opacity-100 pointer-events-none bg-[radial-gradient(farthest-corner_circle_at_var(--m-x)_var(--m-y),_rgba(255,255,255,0.8)_10%,_rgba(255,255,255,0.65)_20%,_rgba(255,255,255,0)_90%)]" />
      </div>
    </div>
  );
};
